...
_ng_write_site(){
  local instance="$1" domains="$2" certs="$3" up_http="$4"
  ...
  # CSP：放行 nexvora.cc（客服 widget）
  local CSP="default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://nexvora.cc https://*.nexvora.cc; style-src 'self' 'unsafe-inline' https://nexvora.cc https://*.nexvora.cc; img-src 'self' data: blob: https://nexvora.cc https://*.nexvora.cc https://*.cloudfront.net https://*.amazonaws.com https://*.gstatic.com https:; connect-src 'self' https://nexvora.cc https://*.nexvora.cc wss://nexvora.cc wss://*.nexvora.cc; frame-src https://nexvora.cc https://*.nexvora.cc; font-src 'self' data: https://nexvora.cc https://*.nexvora.cc;"

  {
    ...
    for i in "${!_doms[@]}"; do
      ...
      echo ""
      echo "    # SSE：关闭缓冲 + 长超时（行情推送）"
      echo "    location /api/client/sse/ {"
...
###############################################################################
setup_nginx_main(){
  local INSTANCE="" DOMAINS="" SSL_CERTS="" UP_HTTP="" UP_HTTPS=""
  while [ $# -gt 0 ]; do
    case "$1" in
  ...
  _ng_write_site "$INSTANCE" "$DOMAINS" "$SSL_CERTS" "$UP_HTTP" || return 1
